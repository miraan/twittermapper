  	...
  	_.each(tweets, function(tweet) {
		...
		_.each(words, function(word) {
			if (!(stopWords.indexOf(word.toLowerCase()) == -1 
				...
				&& word.substring(0,1) != "&" /// remove weird html escape characters from tweets
				&& word.indexOf('/') == -1
				&& word.length > minimumWordLength)) { /// remove links and other sweets
				return;
			}
...
var getWordCloudForProduct = function(product, dateLowerBound, callback) {
	database.getTweets( { product: product, select: 'text', dateLowerBound: dateLowerBound }, function(error, tweets) {
		...
		var array = _.pairs(index);
		var sortedWords = _.sortBy(array, function(pair) { return pair[1]; });
		var minimumWords = 15;
		var maximumWords = 40;
		var optimumWords = Math.round(sortedWords.length / 10);
		if (optimumWords < minimumWords) {
			optimumWords = minimumWords;
		} else if (optimumWords > maximumWords) {
			optimumWords = maximumWords;
		}

		if (optimumWords > sortedWords.length) {
			optimumWords = sortedWords.length;
		}

		var words = [];
		for (var i = sortedWords.length - 1; i >= sortedWords.length - optimumWords; i--) {
			var wordArray = sortedWords[i];
			var word = {};
			word.text = wordArray[0];
			word.size = wordArray[1];
			words.push(word);
		}
